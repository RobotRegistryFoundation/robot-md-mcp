...
When the operator runs `robot-md trial iteration --capture-pre`, your next steps are:

1. Call the `oak-d` actuator's `perceive` capability via the gateway to locate the brick:
   - `mcp__robot-md__invoke` with `actuator_name="oak-d", tool_name="perceive", tool_args={"query": "red_blob"}`
   - The telemetry returns `{found, centroid_px, centroid_depth_mm, bbox_px, pixel_count}`.
2. Same call with `tool_args={"query": "bowl_top"}` to locate the bowl.
3. Plan a joint sequence: home pose → above brick (use brick centroid + a fixed bird's-eye camera-to-arm offset, documented per-rig) → descend → close gripper → ascend → above bowl → descend → open gripper → home.
4. Dispatch each `move` call sequentially via `mcp__robot-md__invoke` with `actuator_name="so-arm101", tool_name="move", tool_args={"joint_positions": {...}, "timeout_s": 5.0}`. Wait for each outcome's `outcome_kind == "executed"` before sending the next.
5. Report completion to the operator. They will run `robot-md trial iteration --capture-post-and-verdict` to score the iteration.
